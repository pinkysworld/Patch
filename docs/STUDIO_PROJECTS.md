...
Project bundle v4 establishes deterministic portable project resources. It does not imply that every resource-consuming control has cross-target parity. Target capability gates remain component-specific: current native Picture follows `native-picture-formats/1.0` (Ready PNG/JPEG, deferred WebP/SVG). Shape, PaintBox Stage 1 and PaintBox `draw image` are Current Ready for PNG/JPEG. Button ImageList images and application/Form icons are Current Ready on Windows, macOS and Linux through Native GUI IR 1.9 / payload v19 / runtime v1.10. Only the older Native GUI IR 1.4 compatibility line fail-closes ImageList, Button image bindings and Form icons.
