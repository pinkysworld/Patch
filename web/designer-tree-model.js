...
function cloneNode(node) {
  ...
  return { labelExpr, ...(node.imageListId && node.imageItem ? { imageListId: String(node.imageListId), imageItem: String(node.imageItem) } : {}), ...(String(node.hint ?? '').trim() ? { hint: String(node.hint).trim() } : {}), ...(String(node.state ?? '').trim() ? { state: String(node.state).trim() } : {}), children };
}
