...
The stress test exposes two concrete boundaries. First, host-backed persistence can resemble a Patch \texttt{set} while remaining outside the Patch commit route. For example, VS Code's \texttt{globalState.update(key, true)} is set-like, yet VS Code Memento performs the persistent update. Second, multi-field Excalidraw \texttt{setState} updates expose grouping that the current single-target model does not claim to commit atomically. These cases are useful precisely because they are not all successes.
