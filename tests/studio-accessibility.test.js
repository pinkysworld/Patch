...
test('result tabs support standard horizontal keyboard navigation', () => {
  for (const marker of ["event.key === 'ArrowRight'", "event.key === 'ArrowLeft'", "event.key === 'Home'", "event.key === 'End'", 'next.focus()', 'next.click()', 'syncResultTabs()']) {
    assert.ok(accessibility.includes(marker), marker);
  }
  assert.match(accessibility, /attributeFilter: \['class'\]/);
  assert.doesNotMatch(accessibility, /attributeFilter: \['class', 'aria-selected'\]/);
});

test('Studio declares and implements keyboard Run and Build shortcuts safely', () => {
  assert.match(html, /id="run"[^>]*aria-keyshortcuts="Control\+Enter Meta\+Enter"/);
  assert.match(html, /id="build"[^>]*aria-keyshortcuts="Control\+Shift\+B Meta\+Shift\+B"/);
  assert.match(accessibility, /event\.ctrlKey \|\| event\.metaKey/);
  assert.match(accessibility, /event\.key === 'Enter'/);
  assert.match(accessibility, /event\.shiftKey && event\.key\.toLowerCase\(\) === 'b'/);
...
test('PWA cache includes accessibility JS and CSS', () => {
  ...
  for (const marker of ['./studio-accessibility.js','./studio-accessibility.css']) {
