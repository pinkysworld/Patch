...
export function validateNativeGuiIRV18(ir) {
  ...
  walkControls(ir, control => {
    ...
    const hasBindingMetadata = Boolean(
      control.imageListId || control.imageItem || control.imageResourceId || control.imageSource ||
      control.imageWidth != null || control.imageHeight != null
    );
    if (!hasBindingMetadata) return;
    if (!control.imageListId || !control.imageItem || !control.imageResourceId || !control.imageSource ||
        control.imageWidth == null || control.imageHeight == null) {
      throw new NativeGuiError(`Native GUI IR 1.8 Button '${control.id}' image binding is incomplete.`);
    }
    const list = listsById.get(normalizeImageListId(control.imageListId));
    if (!list) {
    ...
    const itemName = normalizeImageListItemName(control.imageItem);
    const item = list.items.find(entry => entry.name === itemName);
    if (!item) {
      throw new NativeGuiError(
        `Native GUI IR 1.8 Button '${control.id}' image ${list.id}.${itemName} refers to a missing ImageList item.`
      );
    }
    if (String(control.imageResourceId) !== item.resourceId) {
      throw new NativeGuiError(`Native GUI IR 1.8 Button '${control.id}' image resource does not match ${list.id}.${itemName}.`);
    }
    if (String(control.imageSource) !== item.source) {
      throw new NativeGuiError(`Native GUI IR 1.8 Button '${control.id}' image source does not match ${list.id}.${itemName}.`);
    }
    const width = Number(control.imageWidth);
    const height = Number(control.imageHeight);
    if (!Number.isInteger(width) || !Number.isInteger(height) || width !== list.width || height !== list.height) {
      throw new NativeGuiError(`Native GUI IR 1.8 Button '${control.id}' image size must match ImageList '${list.id}' (${list.width}x${list.height}).`);
    }
  });
  ir.imageLists = lists;
  ...
  const walk = nodes => {
    for (const node of nodes ?? []) {
      if (node.kind === 'uiControl' && node.control === 'imagelist') {
        ...
        lists.push(Object.freeze({ id, width, height, items }));
      }
...
function validateImageLists(value) {
  ...
  return Object.freeze(lists.map(list => {
    ...
    return Object.freeze({ id, width, height, items: validateItems(list.items, id) });
  }));
}
...
function validateItems(value, id) {
  ...
  return Object.freeze(items.map(item => {
    ...
    if (!source) {
      throw new NativeGuiError(`Native GUI IR 1.8 ImageList '${id}' item '${name}' needs an image source.`);
    }
