...
const files = {
  ...
  guardedCalleeExample: read('examples/formal-callee-guard.patch'),
...
mustInclude('README.md', files.readme, [
  ...
  'Beta.29: guard-aware exact structured callee traces', 'Beta.28: exact structured callee traces',
  'GeneratedGuardedCallBodyCertificate.lean', 'GeneratedConcreteCallBodyCertificate.lean',
  'PatchCallBody.lean', 'PatchCallBodyImport.lean', 'GuardExpr', 'evalBoundStmtEqBool_sound',
  'checkedConcreteCallBodyRefinesCallerSignature', 'both branch arms', 'state-dependent guard variables',
  'nested recipe calls', 'production JavaScript/direct-Wasm call equivalence',
  'npm run callee-trace-certify:example', 'npm run guarded-callee-trace-certify:example',
...
mustInclude('web/index.html', files.website, [
  ...
  'GeneratedGuardedCallBodyCertificate.lean', 'GeneratedConcreteCallBodyCertificate.lean',
  'GuardExpr', 'checkedConcreteCallBodyRefinesCallerSignature', 'state-dependent guard variables',
  'production JavaScript/direct-Wasm call equivalence', 'Semantic Window input', 'Window preflight',
...
mustInclude('docs/PATCH_STUDIO.md', files.studio, [
  `What works in 0.2 beta.${beta}`, 'Change IR **0.10**',
  'npm run callee-trace-certify:example', 'npm run guarded-callee-trace-certify:example',
  'GeneratedConcreteCallBodyCertificate.lean', 'GeneratedGuardedCallBodyCertificate.lean',
  'PatchCallBody.lean', 'PatchCallBodyImport.lean', 'GuardExpr',
...
mustInclude('docs/NATIVE_APPS.md', files.native, [
  `Status: **${version}**`, 'Change IR **0.10**',
  'npm run callee-trace-certify:example', 'npm run guarded-callee-trace-certify:example',
  'GeneratedConcreteCallBodyCertificate.lean', 'GeneratedGuardedCallBodyCertificate.lean',
  'sequence/static-repeat', 'production-Wasm call equivalence', 'Window preflight', 'Portable C99', 'FreeBSD 15.1'
]);

mustInclude('docs/ROADMAP.md', files.roadmap, [
  `Current development beta: **${version}**`, '### beta.28: exact structured callee traces',
  '### beta.29: guard-aware exact structured callee traces', 'PatchCallBody.lean', 'PatchCallBodyImport.lean',
  'evalBoundStmtEqBool_sound', 'checkedConcreteCallBodyRefinesCallerSignature',
  'GeneratedConcreteCallBodyCertificate.lean', 'GeneratedGuardedCallBodyCertificate.lean',
...
mustInclude('docs/COMPILER.md', files.compiler, [
  ...
  'Guard-aware structured callee traces: beta.29', 'evalBoundStmt', 'evalBoundStmtEqBool_sound',
  'PatchCallBodyImport.lean', 'GeneratedConcreteCallBodyCertificate.lean', 'GeneratedGuardedCallBodyCertificate.lean',
  'checkedConcreteCallBodyRefinesCallerSignature', 'production JavaScript/direct-Wasm call equivalence'
]);

mustInclude('docs/FORMAL_MODEL.md', files.formal, [
  'Status: **beta.29', 'Beta.28 exact structured callee traces', 'Beta.29 guard-aware exact structured callee traces',
  'BoundStmt', 'GuardExpr', 'evalGuard', 'envOfBindings', 'evalBoundStmt_sound', 'effectListEqBool_sound',
  'evalBoundStmtEqBool_sound', 'boundExecRefinesSignature', 'checkedConcreteCallBodyRefinesCallerSignature',
  'production JavaScript/direct-Wasm call execution'
]);

mustInclude('docs/NOVELTY.md', files.novelty, [
  'Beta.28', 'Beta.29', 'structured execution traces', 'guard-aware exact callee traces',
  'GeneratedConcreteCallBodyCertificate.lean', 'GeneratedGuardedCallBodyCertificate.lean',
  'not a new novelty headline', 'production JavaScript/direct-Wasm call equivalence'
]);

mustInclude('paper/README.md', files.paper, [
  `Patch ${version} / Change IR 0.10`, 'Beta.28 exact structured callee-trace milestone',
  'Beta.29 guard-aware exact callee-trace milestone', 'GeneratedConcreteCallBodyCertificate.lean',
  'GeneratedGuardedCallBodyCertificate.lean', 'PatchCallBody.lean', 'PatchCallBodyImport.lean',
  'production JavaScript/direct-Wasm call equivalence', 'full compiler verification'
]);
...
mustInclude('src/concrete-call-witness.js', files.concreteWitness, [
  'buildConcreteCallWitnesses', 'patch-concrete-call-witness', 'expectedCalleeEnv', 'abstractArgRanges',
...
mustInclude('src/concrete-call-body-certificate.js', files.concreteBodyCertificate, [
  ...
  'BoundStmt.branch', 'GuardExpr.', 'evalBoundStmtEqBool', 'boundBodyCoveredBool', 'signatureCoversBool',
...
mustInclude('scripts/generate-concrete-call-body-certificate.js', files.concreteBodyGenerator, [
  'generateConcreteCallBodyCertificate', 'certified structured call(s)', 'GuardExpr branch truth', 'both branch arms'
]);
mustInclude('examples/formal-callee-trace.patch', files.calleeTraceExample, ['bonus + 1', 'repeat 2', 'amount * 2']);
mustInclude('examples/formal-callee-guard.patch', files.guardedCalleeExample, ['if amount >= 3:', 'caller_high', 'caller_low', 'amount * 2']);
...
mustInclude('formal/PatchCallBody.lean', files.callBody, [
  'import PatchGuarded', 'inductive BoundStmt', '| branch (guard : GuardExpr)', 'BoundExec.branchThen', 'BoundExec.branchElse',
  'evalGuard guard (envOfBindings bindings)', 'def evalBoundStmt', 'theorem evalBoundStmt_sound',
...
mustInclude('.github/workflows/formal.yml', files.formalWorkflow, [
  ...
  'GeneratedConcreteCallBodyCertificate.lean', 'GeneratedGuardedCallBodyCertificate.lean', 'PatchCallBodyImport',
  "github.event.pull_request.draft == false", 'cancel-in-progress: true'
]);
mustInclude('.github/workflows/ci.yml', files.ciWorkflow, [
  'callee-trace-certify:example', 'guarded-callee-trace-certify:example', 'src/concrete-call-body-certificate.js',
  "github.event.pull_request.draft == false", 'cancel-in-progress: true'
]);
mustInclude('.github/workflows/beta29-guarded-callee-traces.yml', files.beta29Workflow, [
  'Patch Beta29 Guarded Callee Traces', 'GeneratedGuardedCallBodyCertificate.lean', 'GeneratedConcreteCallBodyCertificate.lean',
  'PatchCallBodyImport', "github.event.pull_request.draft == false", 'cancel-in-progress: true'
]);
...
})) rejectOtherCurrentProductBeta(name, content, beta);
...
// Historical milestone headings such as "Beta.28" are valid. Only reject stale
// fully-qualified/current-product forms like 0.2.0-beta.28 or 0.2 beta.28.
function rejectOtherCurrentProductBeta(name, content, expectedBeta) {
  const found = [...content.matchAll(/0\.2\.0-beta\.(\d+)|0\.2 beta\.(\d+)/g)].map(match => match[1] ?? match[2]);
  for (const value of found) if (value !== expectedBeta) throw new Error(`${name} contains stale current-product beta ${value}; expected ${expectedBeta}`);
}
