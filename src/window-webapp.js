...
export const PATCH_WINDOW_WEB_VERSION = '0.9';
...
export function buildStandaloneWindowWebApp(compiled, name) {
  ...
  const support = validateWindowRuntimeSupport(compiled, { allowTree: true, allowTreeNodeImages: true, allowTreeNodeHints: true, allowTreeNodeStates: true, allowListView: true, allowSlider: true, allowProgressBar: true, allowScrollBar: true, allowAdvancedTableColumns: true, allowMemo: true, allowPaintBox: true, allowImageList: true });
  ...
  const metadata = {
    ...
    ...(support.listViews ? {
      listViewStage: 1,
      listViewVersion: '0.1',
      listViewModes: ['icons', 'details'],
      listViewEventValue: 'selected-label-text',
      listViewCount: support.listViews
    } : {})
  };
...
function uiListViewItems(items,lists=new Map()){return(items??[]).map(item=>{const list=item.imageListId?lists.get(item.imageListId):null;const image=item.imageListId&&item.imageItem?(list?.items??[]).find(entry=>entry.name===item.imageItem):null;const model={label:uiText(item.labelExpr),detail:item.detail?String(item.detail):''};if(image){model.imageSource=uiText(image.sourceExpr);model.imageWidth=Number(list?.logicalWidth)||16;model.imageHeight=Number(list?.logicalHeight)||16;}return model;});}
