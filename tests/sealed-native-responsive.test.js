...
test('sealed GUI v8 advances only the layout transport while v7 remains the default contract', () => {
  ...
  const sealed8 = sealNativeGuiRuntime(fakePe, gui, { version: 8 });
...
test('legacy sealing scripts stay pinned to payload v7 unless a runtime workflow explicitly opts into v8', () => {
...
test('sealed Win32 v0.9 consumes payload v8 and responds to WM_SIZE', () => {
  for (const marker of [
    'version != 8',
    'PatchConvertPayloadV8ToV7',
    'ApplyPatchResponsiveLayoutV09',
    'WM_SIZE',
    'MoveWindow',
    'RunPatchResponsiveSmokeV09',
