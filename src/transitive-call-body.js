...
export function buildTransitiveCallBodyWitnesses(ast, formalCalls) {
  ...
  for (const call of exactCalls.witnesses ?? []) {
    ...
    const built = buildRecipeTree(callee, calleeEntry, recipes, formalCalls, new Set([callee.name]));
    if (!built.supported) {
...
function buildRecipeTree(recipe, entry, recipes, formalCalls, active) {
  ...
  return buildBlock(
    recipe.body ?? [], recipe, entry, rangeBindings, new Set(Object.keys(rangeBindings)),
    recipes, formalCalls, active
  );
}
...
function buildNestedCall(node, recipe, entry, rangeBindings, recipes, formalCalls, active) {
  ...
  return {
    supported: true,
    stmt: {
      kind: 'call', callee: callee.name, argExprs, params: [...params], declared,
      calleeSignature: calleeEntry.signature ?? [], body: nestedBody.stmt
    }
  };
}
  ...
  switch (stmt?.kind) {
    ...
    case 'call': {
      ...
      for (let index = 0; index < values.length; index += 1) {
        const value = values[index]; const range = stmt.declared[index];
...
function evalRangeExprExact(expr, env) {
  ...
  switch (expr?.kind) {
    ...
    case 'var': if (!Object.hasOwn(env, expr.name)) throw new Error(`exact environment has no binding for '${expr.name}'`); value = env[expr.name]; break;
...
function rangeBindingsFor(entry) { return Object.fromEntries((entry?.paramRanges ?? []).map(item => [item.name, item.range])); }
function declaredRangesFor(entry, params) {
  const byName = new Map((entry?.paramRanges ?? []).map(item => [item.name, item.range])); const out = [];
...
function parseStaticRepeat(expr) {
  const text = String(expr ?? '').trim(); if (!/^(0|[1-9][0-9]*)$/.test(text)) return null;
  const value = Number(text); return Number.isSafeInteger(value) && value <= 10000 ? value : null;
}
...
function sameEffect(left, right) {
  return left?.target === right?.target && (left?.field ?? null) === (right?.field ?? null) &&
    left?.operation === right?.operation && left?.amountRange?.min === right?.amountRange?.min &&
    left?.amountRange?.max === right?.amountRange?.max;
}
function unsupported(node, reason) { return { supported: false, reason: `line ${node?.line ?? '?'}: ${reason}` }; }
