...
test('Table and TreeView share the bounded keyed runtime selection contract', () => {
  ...
  assert.match(renderer, /from '\.\/studio-runtime-selection-state\.js'/);
  assert.match(renderer, /getRuntimeSelection\(context\.container, 'tree', key\)/);
  assert.match(renderer, /setRuntimeSelection\(context\.container, 'tree', key, selectedPath\)/);
  assert.match(renderer, /patchRuntimeSelectionKind = 'tree'/);
