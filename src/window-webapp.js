...
export const PATCH_WINDOW_WEB_VERSION = '0.6';

const WINDOW_WEB_SUPPORTED = new Set([
  'create', 'createThing', 'window', 'uiControl', 'event', 'openForm', 'closeForm', 'allow', 'show',
...
function buildUI(){return windows.map((windowNode,index)=>({id:windowNode.id||('window'+(index+1)),visible:windowNode.id?formVisibility.get(windowNode.id)!==false:true,title:uiText(windowNode.titleExpr),controls:(windowNode.body??[]).filter(node=>node.kind==='uiControl').map(node=>({type:node.control,id:node.id,text:node.textExpr?uiText(node.textExpr):'',options:Array.isArray(node.options)?node.options.map(uiOption):[],value:node.id&&state.has(node.id)?clone(state.get(node.id)):''}))}));}
function controlType(id){for(const windowNode of windows){const control=(windowNode.body??[]).find(node=>node.kind==='uiControl'&&node.id===id);if(control)return control.control;}return null;}
function trigger(control,event='clicked',payload={}){output=[];const matches=events.filter(x=>x.control===control&&x.event===event);if(!matches.length)throw new PatchAppError("There is no '"+event+"' action for '"+control+"'.");let locals={};if(event==='changed'){if(!Object.prototype.hasOwnProperty.call(payload,'value'))throw new PatchAppError("The 'changed' action for '"+control+"' needs an event-local value.");const type=controlType(control);if(type==='checkbox'&&typeof payload.value!=='boolean')throw new PatchAppError("The 'changed' action for checkbox '"+control+"' needs a Boolean event-local value.");if(type==='combo'&&typeof payload.value!=='string')throw new PatchAppError("The 'changed' action for combo '"+control+"' needs a text event-local value.");locals={value:payload.value};}for(const handler of matches)executeBlock(handler.body,{...locals});render();showOutput();}
function render(){const models=buildUI();appEl.innerHTML='';if(!models.length){appEl.innerHTML='<section class="console"><strong>No Patch window is defined.</strong></section>';return;}for(const model of models){const shell=document.createElement('section');shell.className='window';shell.hidden=model.visible===false;const title=document.createElement('header');title.textContent=model.title;const body=document.createElement('div');body.className='body';for(const control of model.controls){if(control.type==='text'){const el=document.createElement('p');el.className='text';el.textContent=control.text;body.append(el);}else if(control.type==='button'){const el=document.createElement('button');el.textContent=control.text;el.addEventListener('click',()=>safeTrigger(control.id,'clicked'));body.append(el);}else if(control.type==='input'){const el=document.createElement('input');el.value=control.value??'';el.placeholder=control.text||control.id||'';el.addEventListener('input',()=>safeTrigger(control.id,'changed',{value:el.value}));body.append(el);}else if(control.type==='checkbox'){const label=document.createElement('label');label.className='checkbox';const el=document.createElement('input');el.type='checkbox';el.checked=control.value===true;const text=document.createElement('span');text.textContent=control.text;el.addEventListener('change',()=>safeTrigger(control.id,'changed',{value:el.checked}));label.append(el,text);body.append(label);}else if(control.type==='combo'){const el=document.createElement('select');for(const option of control.options??[]){const item=document.createElement('option');item.value=option;item.textContent=option;el.appendChild(item);}el.value=String(control.value??'');el.addEventListener('change',()=>safeTrigger(control.id,'changed',{value:el.value}));body.append(el);}}shell.append(title,body);appEl.append(shell);}}
