...
static bool ParsePayload(const std::vector<uint8_t>&bytes){
  try{
    ...
    gTabPages.resize(gControls.size(),nil);gRadioItems.resize(gControls.size(),nil);return true;
  }catch(...){return false;}
}
