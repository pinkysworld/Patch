...
function buildUI(){return windows.map((windowNode,index)=>({id:windowNode.id||('window'+(index+1)),visible:windowNode.id?formVisibility.get(windowNode.id)!==false:true,title:uiText(windowNode.titleExpr),controls:buildUIItems(windowNode.body,imageListsFrom(windowNode.body))}));}
