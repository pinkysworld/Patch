...
test('Input presentation contract v0.4 keeps plain/password/date/time/calendar modes explicit', () => {
  assert.equal(PATCH_INPUT_PRESENTATION_VERSION, '0.4');
  assert.deepEqual(patchInputPresentationModes(), ['plain', 'password', 'date', 'time', 'calendar']);
  ...
  assert.equal(normalizePatchInputPresentation(' CALENDAR '), 'calendar');
  assert.throws(() => normalizePatchInputPresentation('secret'), /Use plain, password, date, time or calendar/);
});
...
test('PasswordEdit source metadata is transparent and round-trippable', () => {
  ...
  assert.equal(parsePatchInputPresentationDirective('# @input-mode calendar'), 'calendar');
  ...
  assert.equal(formatPatchInputPresentationDirective('calendar'), '# @input-mode calendar');
...
test('PasswordEdit changes presentation only and maps to the browser password input type', () => {
  ...
  assert.equal(patchInputDomType('calendar'), 'text');
});
...
test('PasswordEdit Stage 1 support is explicit and native targets remain fail-closed', () => {
  ...
  assert.equal(patchInputPresentationTargetSupport('calendar').web, 'supported');
...
`;
  ...
  assert.equal(PATCH_WINDOW_INPUT_PRESENTATION_VERSION, '0.4');
