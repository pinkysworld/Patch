...
Empty Designer and App panes use a compact guidance card. The status bar stays visible on narrow screens and carries save state plus the current Ready contract chip **IR 1.9 / v1.10**.
...
- Shape has Studio, Standalone Web and current native Ready support, introduced on IR 1.5 / payload v15 / runtime v1.6 and preserved by the current IR 1.9 / payload v19 / runtime v1.10 line;
- PaintBox Stage 1 clear/line/rectangle/ellipse/text and PNG/JPEG `draw image` have Studio, Standalone Web and current native Ready support;
- Button ImageList images are Current Ready on Windows, macOS and Linux; only the older IR 1.4 compatibility line fail-closes ImageList and Button images;
- Window icons are source-backed Studio/Web chrome and favicon packaging under `window-icon/1.0`, and Current Ready application/Form icons on Windows, macOS and Linux. Only the older IR 1.4 compatibility line fail-closes Form icons.
