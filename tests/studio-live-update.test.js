...
test('site builder content-addresses the complete transitive browser module graph including multiline imports', () => {
  ...
  assert.ok(currentFacade.includes(`from './native-gui-ir-v18.js?v=${revision}'`), 'multiline current facade IR import must be revisioned');
  assert.ok(currentFacade.includes(`from './native-gui-ir-v17.js?v=${revision}'`), 'multiline current facade still revisiones IR 1.7 import');
  assert.ok(currentFacade.includes(`from './sealed-native-gui-v18.js?v=${revision}'`), 'multiline current facade sealer import must be revisioned');
