...
test('Designer control picker exposes every existing top-level toolbox control exactly once', () => {
  const expected = ['addText','addButton','addInput','addCheckbox','addRadio','addCombo','addListbox','addSlider','addTable','addTree','addTabs','addPanel','addPicture','addShape','addStatusbar','addTimer'];
...
test('Designer control picker groups controls by user-facing purpose', () => {
  ...
  assert.deepEqual(groups.find(group => group.group === 'Graphics').tools.map(tool => tool.label), ['Picture','Shape']);
