...
test('raw call-site validation binds caller callee line and exact argument texts', () => {
  const source = `make leaf(a number 0..10, b number 0..20):
  show a
...
test('concrete call certification fails closed when raw call-site validation is invalid', () => {
  const source = `make leaf(amount number 0..5):
  show amount
...
test('all shipped examples preserve exact raw-source call-site identity', () => {
  ...
  for (const name of fs.readdirSync(dir).filter(value => value.endsWith('.patch')).sort()) {
    ...
    if (!compiled.ir.callSiteValidation.validated) {
      mismatches.push(`${name}: ${compiled.ir.callSiteValidation.reasons.join(' | ')}`);
    }
  }
