...
const buildMetadata = {
  ...
  namedForms: support.namedForms,
  controls: support.controls,
  events: support.events,
  formActions: support.formActions,
...
function mainProcessSource(name) {
  return `const { app, BrowserWindow } = require('electron');\nconst path = require('node:path');\nconst smoke=process.argv.includes('--patch-smoke');\napp.whenReady().then(() => {\n  const win = new BrowserWindow({\n    width: 900, height: 640, minWidth: 480, minHeight: 360,\n    title: ${JSON.stringify(name)}, show: !smoke,\n    webPreferences: { contextIsolation: true, nodeIntegration: false, sandbox: true }\n  });\n  win.webContents.setWindowOpenHandler(()=>({action:'deny'}));\n  win.webContents.on('will-navigate',event=>event.preventDefault());\n  if(smoke){\n    win.webContents.once('did-finish-load',async()=>{\n      try{\n        const ok=await win.webContents.executeJavaScript("new Promise(resolve=>setTimeout(()=>resolve(Boolean(document.documentElement.dataset.patchCompiled==='1'&&document.querySelector('.window')&&!document.querySelector('[data-patch-build-error]'))),100))");\n        if(!ok)throw new Error('Packaged Patch GUI did not execute its compiled Window artifact.');\n        const hasNavigation=await win.webContents.executeJavaScript("Boolean(document.querySelector('[data-form-id=settings]')&&document.querySelector('[data-control-id=open_settings]'))");\n        if(hasNavigation){\n          const initial=await win.webContents.executeJavaScript("(()=>{const main=document.querySelector('[data-form-id=main]');const settings=document.querySelector('[data-form-id=settings]');return Boolean(main&&!main.hidden&&settings&&settings.hidden);})()");\n          if(!initial)throw new Error('Packaged Patch GUI named Forms have incorrect initial visibility.');\n          const opened=await win.webContents.executeJavaScript("(()=>{document.querySelector('[data-control-id=open_settings]').click();return new Promise(resolve=>setTimeout(()=>{const settings=document.querySelector('[data-form-id=settings]');resolve(Boolean(settings&&!settings.hidden));},50));})()");\n          if(!opened)throw new Error('Packaged Patch GUI could not open its Settings Form.');\n          const closed=await win.webContents.executeJavaScript("(()=>{document.querySelector('[data-control-id=close_settings]').click();return new Promise(resolve=>setTimeout(()=>{const settings=document.querySelector('[data-form-id=settings]');resolve(Boolean(settings&&settings.hidden));},50));})()");\n          if(!closed)throw new Error('Packaged Patch GUI could not close its Settings Form.');\n        }\n        app.quit();\n      }catch(error){console.error(error?.stack||error?.message||String(error));process.exitCode=1;app.quit();}\n    });\n  }\n  win.loadFile(path.join(__dirname, 'player.html'));\n}).catch(error=>{console.error(error?.stack||error?.message||String(error));process.exitCode=1;app.quit();});\napp.on('window-all-closed', () => { if (process.platform !== 'darwin') app.quit(); });\n`;
}
