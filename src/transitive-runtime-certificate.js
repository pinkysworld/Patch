...
export const PATCH_TRANSITIVE_RUNTIME_CERTIFICATE_VERSION = '0.2';

/**
 * Generate one self-contained Lean file containing the beta.30 exact call-tree
 * certificate followed by beta.32 observed direct-Wasm invocation-frame checks.
 *
 * JavaScript executes Wasm and performs whole-trace independent validation.
 * The validator reconstructs concrete DO frames without backend call markers.
 * Lean checks that the selected frame binding equals the beta.30 exact callee
 * BindingList and then re-evaluates the frame-selected observed effect list
 * through `evalCallTreeStmtEqBool` before caller-signature refinement.
 */
export async function generateTransitiveRuntimeCertificate(source, options = {}) {
  ...
  if (beta30.sourceSha256 !== runtime.sourceSha256) {
    throw new Error(`Beta.30/beta.32 source evidence mismatch: ${beta30.sourceSha256} != ${runtime.sourceSha256}.`);
  }
  if (beta30.artifact?.version !== runtime.transitiveWitnessVersion) {
    throw new Error(`Beta.30/beta.32 transitive witness schema mismatch: ${beta30.artifact?.version ?? 'missing'} != ${runtime.transitiveWitnessVersion ?? 'missing'}.`);
  }
  if (runtime.runtimeValidation?.ok !== true) {
    throw new Error('Beta.32 runtime certificate requires successful complete direct-Wasm semantic-effect validation.');
  }
  if (!runtime.invocationFrameVersion) {
    throw new Error('Beta.32 runtime certificate requires independently reconstructed invocation-frame evidence.');
  }
  ...
  if (!supported.length) {
    ...
    throw new Error(`No beta.32 invocation-frame direct-Wasm/transitive-call correspondence is certifiable.${reasons.length ? ` ${reasons.join('; ')}` : ''}`);
  }
  ...
  for (const item of supported) {
    ...
    if (!beta30.certified.includes(beta30Label)) {
      throw new Error(`Beta.32 correspondence ${beta30Label} has no matching beta.30 generated certificate theorem.`);
    }
    if (!item.frame || !Number.isSafeInteger(item.frameId) || item.frameId < 0) {
      throw new Error(`Beta.32 correspondence ${beta30Label} has no concrete invocation frame.`);
    }
    ...
    blocks.push(`def ${runtimeId}_occurrenceIds : List Nat := ${leanList((item.occurrenceIds ?? []).map(value => String(value)))}`);
    blocks.push(`def ${runtimeId}_frameId : Nat := ${item.frameId}`);
    blocks.push(`def ${runtimeId}_frameDepth : Nat := ${item.frame.depth}`);
    blocks.push(`def ${runtimeId}_frameInvocation : Nat := ${item.frame.invocation}`);
    blocks.push(`def ${runtimeId}_frameBindings : BindingList := ${leanBindingList(item.frame.bindings)}`);
  ...
  if (beta30Lean === beta30.lean) {
    throw new Error('Could not upgrade beta.30 generated import to PatchCallRuntime for beta.32 certificate generation.');
  }
  const runtimeLean = `\n\nopen PatchFormal\n\nnamespace PatchGeneratedTransitiveRuntimeCertificate\n\nopen PatchGeneratedTransitiveCallBodyCertificate\n\n/-- Beta.32 evidence is generated only after the real direct-Wasm module has\n    executed and the entire raw target/before/after transition trace has matched\n    the independent Change-IR execution model. The validator reconstructs recipe\n    scope, semantic operations and concrete DO invocation frames without backend\n    call-enter/call-exit markers. A beta.30 witness is linked to one concrete\n    frame by caller/callee/dynamic invocation identity and exact frame bindings.\n    Lean checks that the independently reconstructed frame BindingList equals the\n    beta.30 exact BindingList, then re-evaluates the frame-selected observed\n    effect list against the exact call tree before caller-signature refinement.\n    Runtime capture and independent-validator/frame-reconstruction correctness\n    remain explicit proof-free evidence boundaries. -/\ndef runtimeSourceSha256 : String := ${leanString(runtime.sourceSha256)}\ndef directWasmTraceSha256 : String := ${leanString(runtime.runtimeTraceSha256)}\ndef transitiveRuntimeCorrespondenceVersion : String := ${leanString(runtime.version)}\ndef transitiveRuntimeCertificateVersion : String := ${leanString(PATCH_TRANSITIVE_RUNTIME_CERTIFICATE_VERSION)}\ndef directEffectValidationVersion : String := ${leanString(runtime.directEffectValidationVersion)}\ndef invocationFrameVersion : String := ${leanString(runtime.invocationFrameVersion)}\ndef runtimePatchIrVersion : String := ${leanString(runtime.irVersion)}\n\n${blocks.join('\n\n')}\n\nend PatchGeneratedTransitiveRuntimeCertificate\n`;
  ...
  return {
    ...
    invocationFrameVersion: runtime.invocationFrameVersion
  };
}

function leanBindingList(bindings) {
  if (!Array.isArray(bindings)) throw new Error('Beta.32 frame bindings must be an array.');
  return leanList(bindings.map(item => {
    if (!item || typeof item.name !== 'string' || !Number.isSafeInteger(item.value)) {
      throw new Error('Beta.32 frame binding is outside the exact safe-integer BindingList fragment.');
    }
    return `(${leanString(item.name)}, ${leanInt(item.value)})`;
  }));
}

function leanEffect(effect) {
  if (!['increase', 'decrease'].includes(effect?.operation)) {
    throw new Error(`Beta.32 runtime effect '${effect?.operation}' is outside exact quantitative correspondence.`);
  }
  ...
  if (!range || !Number.isSafeInteger(range.min) || !Number.isSafeInteger(range.max) || range.min !== range.max || range.min < 0) {
    throw new Error('Beta.32 runtime quantitative effect must carry one exact non-negative safe-integer interval.');
  }
