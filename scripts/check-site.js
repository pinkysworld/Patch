...
for (const id of ['code', 'run', 'build', 'buildTarget', 'output', 'ir', 'app', 'designer', 'designerCanvas', 'addText', 'addButton', 'addInput', 'projectName', 'projectKind']) {
  if (!html.includes(`id="${id}"`)) throw new Error(`Patch Studio is missing required element #${id}`);
}
for (const phrase of ['One language. One Studio. Many targets.', 'iPhone & iPad', 'Research project', 'State-Change Factorization', 'Roadmap']) {
...
for (const cached of ["'./src/compiler.js'", "'./src/wasm.js'", "'./src/designer.js'"]) {
