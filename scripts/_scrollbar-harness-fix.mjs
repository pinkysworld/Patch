...
const oldDocsProgress = "assert.match(roadmap, /ProgressBar as passive number-backed Slider/);";
const newDocsProgress = "assert.match(roadmap, /ProgressBar as passive number-backed Slider presentation via `# @slider-mode progress`/);";
if (!source.includes(oldDocsProgress)) throw new Error('ScrollBar roadmap acceptance harness anchor is missing.');
source = source.replaceAll(oldDocsProgress, newDocsProgress);
