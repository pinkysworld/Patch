...
if (!sourcePath) {
  console.error('Use: node scripts/build-native-win32.js program.patch AppName dist [--emit-only] [--smoke] [--tree-v13] [--slider-v14]');
  process.exit(2);
}
if (tableV09 || menuV10 || menuV11 || listV12) {
  console.error('Direct native builds no longer select retired v07–v11 contracts. Use --tree-v13 (frozen) or --slider-v14 (current).');
  process.exit(2);
}
...
const plan = buildNativeGuiPlan(compiled, { treeV13, sliderV14 });
const gui = plan.gui;
...
fs.writeFileSync(metadataPath, JSON.stringify({
  format: 'patch-native-win32-build', version: '0.1', backendVersion, appName,
  nativeGuiIrVersion: gui.version, changeIrVersion: compiled.ir?.version ?? null,
  forms: gui.forms.length, controls: controlCount, events: gui.events.length,
  sourceSha256: createHash('sha256').update(source, 'utf8').digest('hex'),
  shell: 'native-win32', electron: false, crt: 'static', nativeGuiTier: plan.tier,
...
if (process.platform !== 'win32') { console.error('Direct Win32 linking currently runs on Windows. Use --emit-only to inspect generated C++ elsewhere.'); process.exit(3); }

const compiler = resolveMsvc();
const objectPath = path.join(outDir, `${appName}.obj`);
const clArgs = ['/nologo','/EHsc','/std:c++17','/O2','/MT','/utf-8',`/Fo${objectPath}`,cppPath,'user32.lib','gdi32.lib','comctl32.lib','comdlg32.lib','/link','/SUBSYSTEM:WINDOWS',`/OUT:${exePath}`];
const result = compiler.kind === 'direct' ? spawnSync(compiler.command, clArgs, { stdio:'inherit', windowsHide:true }) : runThroughVsDevCmd(compiler.vsDevCmd, clArgs);
...
function emitForTier(plan) { if(plan.tier==='slider-v14')return emitWin32GuiCppV14(plan.gui); return emitWin32GuiCppV13(plan.gui); }
function backendVersionForTier(tier) { if(tier==='slider-v14')return PATCH_WIN32_GUI_BACKEND_V14_VERSION; return PATCH_WIN32_GUI_BACKEND_V13_VERSION; }
...
function safeName(name){const cleaned=String(name).trim().replace(/[^A-Za-z0-9_-]/g,'_').replace(/_+/g,'_').slice(0,64);return cleaned||'PatchNativeWindow';}function batchQuote(value){return `"${String(value).replace(/"/g,'""')}"`;}
