...
const requireAll = (label, text, markers) => { for (const marker of markers) if (!text.includes(marker)) throw new Error(`${label} is missing beta36 marker: ${marker}`); };
const rejectAll = (label, text, markers) => { for (const marker of markers) if (text.includes(marker)) throw new Error(`${label} contains beta35/current-contract drift: ${marker}`); };
...
requireAll('beta36 Studio', index, [
  'data-patch-version="0.2.0-beta.36"','0.2 beta.36+','Native GUI IR 1.9','payload v19','runtime v1.10',
...
requireAll('beta36 current native facade', current, [
  "native-gui-1.9/payload-19/runtime-1.10","PATCH_CURRENT_NATIVE_RUNTIME_VERSION = '1.10'",
...
requireAll('beta36 downloads', downloads, [
  'offline-compiler-v0.2','Native GUI IR <strong>1.9</strong>','payload <strong>v19</strong>','runtime <strong>v1.10</strong>',
...
console.log('beta.36 Studio surface gate passed for Current Ready IR 1.9 / payload v19 / runtime v1.10.');
