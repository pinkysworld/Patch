...
function ensureStyles() {
  if (document.querySelector('style[data-patch-progressbar-stage1]')) return;
  const style = document.createElement('style');
  style.dataset.patchProgressbarStage1 = '1';
  style.textContent = `
.patch-slider.patch-progressbar-studio{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:8px 10px;min-width:180px}
...
async function addProgressBarFromStudio(event) {
...
function installObservers() {
  ...
  if (formSelect?.dataset.patchProgressbarObserver !== '1') {
    ...
    formSelect.addEventListener('change', scheduleSync);
  }
}
