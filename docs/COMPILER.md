...
Things (`CREATE_THING`), text/boolean/list state and GUI execution are outside that numeric subset and fail closed rather than silently falling back to the interpreter. They remain outside the beta.32 Lean runtime-correspondence claim. Runtime Thing storage in the interpreter and Window Web path is prototype-free. `patch doctor` self-checks a tiny numeric program through the interpreter, direct Wasm and C99, then checks that a Thing program still fails closed on those backends.
