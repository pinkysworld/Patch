...
static std::vector<NSImage*> gPatchButtonNativeImagesV19;
...
static NSImage* PatchCreateButtonImageV19(const PatchButtonImageConsumerV19& consumer) {
  if (consumer.assetIndex >= gPatchButtonImageAssetsV19.size()) return nil;
  ...
  if (!image || image.size.width <= 0 || image.size.height <= 0) { if (image) [image release]; return nil; }
  [image setSize:NSMakeSize((CGFloat)consumer.logicalWidth, (CGFloat)consumer.logicalHeight)];
  return image;
}

static bool PatchInstallButtonImagesV19() {
  gPatchButtonNativeImagesV19.assign(gControls.size(), nil);
  for (const auto& item : gPatchButtonImagesV19) {
    auto& control = gControls[(size_t)item.nativeIndex];
    if (control.kind != CK_BUTTON || ![control.widget isKindOfClass:[NSButton class]]) return false;
    NSButton* button = (NSButton*)control.widget;
    NSImage* image = PatchCreateButtonImageV19(item);
    if (!image) return false;
    button.image = image;
    button.imagePosition = NSImageLeft;
    button.imageScaling = NSImageScaleProportionallyDown;
    gPatchButtonNativeImagesV19[(size_t)item.nativeIndex] = image;
  }
  return true;
}

static void PatchDestroyButtonImagesV19() {
  for (NSImage* image : gPatchButtonNativeImagesV19) if (image) [image release];
...
static int RunPatchButtonImageSmokeV19() {
  ...
  for (const auto& item : gPatchButtonImagesV19) {
    auto& control = gControls[(size_t)item.nativeIndex];
    if (control.kind != CK_BUTTON || ![control.widget isKindOfClass:[NSButton class]]) return code++;
    NSButton* button = (NSButton*)control.widget;
    if (!button.image || button.imagePosition != NSImageLeft) return code++;
    NSSize size = button.image.size;
    if ((uint32_t)llround(size.width) != item.logicalWidth || (uint32_t)llround(size.height) != item.logicalHeight) return code++;
    if (![button.title isEqualToString:RenderText(control.text)]) return code++;
  }
  return 0;
}
...
int main(int argc, const char* argv[]) {
  @autoreleasepool {
    ...
    if (gSmokeMode) {
      ...
      PatchDestroyButtonImagesV19(); PatchDestroyPaintImagesV18();
      return result;
    }
