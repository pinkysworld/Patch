...
function requireText(text, marker, label) {
  if (!text.includes(marker)) throw new Error(`Studio Ready v1.2 site check failed: ${label} is missing '${marker}'.`);
}
function rejectText(text, marker, label) {
  if (text.includes(marker)) throw new Error(`Studio Ready v1.2 site check failed: ${label} still contains '${marker}'.`);
}
...
requireText(index, 'payload v11', 'Studio native status');
requireText(index, 'Runtime v1.2', 'Studio native status');
...
requireText(nativeBuild, 'PATCH_SEALED_NATIVE_GUI_MENU_VERSION', 'browser native builder');
...
requireText(docs, 'payload v11/runtime v1.2', 'Documentation current list contract');

console.log('Patch Studio Ready v1.2 public-site runtime surface validated.');
