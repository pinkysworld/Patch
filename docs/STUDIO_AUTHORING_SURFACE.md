...
The Designer exposes Text, Button, Input, Checkbox, Radio, ComboBox, ListBox, Table, TreeView and Tabs through the same source-backed toolbox boundary.
...
- add Text, Button, Input, Checkbox, Radio, ComboBox, ListBox, Table and TreeView;
- remove;
- move up/down;
- duplicate;
...
This is the complete current authoring surface for the **existing Patch UI/control vocabulary**. It does not mean every conceivable IDE feature exists.

The following remain separate future work rather than missing implementations of this surface:

- new/richer data controls beyond the current Table, ListBox and TreeView vocabulary; those require explicit language/IR/runtime/native design rather than a Studio-only widget;
- manual assistive-technology verification with Narrator, VoiceOver, Orca and comparable tools; the repository has an automated keyboard/focus baseline but makes no WCAG conformance claim;
- distribution work such as installer/uninstaller formats, credentialed Windows signing evidence, macOS signing/notarization evidence and possible future FreeBSD native GUI support;
- research/evaluation roadmap items unrelated to Studio authoring.
