...
test('ComboBox changed value is transient text until Patch source explicitly changes state', () => {
  ...
  assert.throws(() => triggerWindowEvent(runtime, 'size', 'changed', { value: true }), /needs a text event-local value/);
});
...
test('Designer can add, move, rename and edit ComboBox options in source', () => {
  ...
  edited = updateDesignerControl(edited, combo, { id: 'size', options: ['"Small"', '"Medium"', '"Large"'], x: 40, y: 80, width: 260, height: 38 });
...
test('native GUI v0.4 carries ComboBox options, text binding and changed event semantics', () => {
  ...
  assert.deepEqual(ir.events, [{ control: 'size', event: 'changed', valueType: 'text', actions: [{ kind: 'change', target: 'size', stateType: 'text', ops: [{ op: 'set', value: { kind: 'eventValue' } }] }] }]);
});
