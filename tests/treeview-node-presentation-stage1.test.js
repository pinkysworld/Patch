...
test('TreeView node presentation 0.1 has deterministic optional ImageList.item syntax', () => {
  ...
  assert.deepEqual(parsePatchTreeNodeDeclaration('node "image tree_icons.folder"'), {
    labelExpr: '"image tree_icons.folder"', imageListId: null, imageItem: null
  });
});
...
test('Designer tree mutations preserve and edit source-backed node image metadata', () => {
  ...
  const duplicated = duplicateTreeSubtree(result.nodes, [0]);
  assert.equal(duplicated.nodes[1].imageListId, 'tree_icons');
  assert.equal(duplicated.nodes[1].imageItem, 'folder');
  assert.equal(duplicated.nodes[1].children[0].imageItem, 'folder');
});
...
test('Studio and Standalone Web render TreeView node icons without changing changed(value)', () => {
  ...
  assert.match(webapp, /uiTreeNodesWithImages\(node\.treeNodes,lists\)/);
  assert.doesNotMatch(webapp, /uiTreeNodes\(node\.treeNodes\)/);
...
test('TreeView node presentation helper ships in the hosted/offline Studio source graph', () => {
  const build = fs.readFileSync('scripts/build-site.js', 'utf8');
  const sw = fs.readFileSync('web/sw.js', 'utf8');
  assert.match(build, /tree-node-presentation\.js/);
  assert.match(sw, /\.\.\/src\/tree-node-presentation\.js/);
});
