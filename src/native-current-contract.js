...
function assertCurrentNativeInputPresentation(nodes) {
  for (const node of nodes ?? []) {
    if (node?.kind === 'uiControl' && node.control === 'input') {
      ...
      if (node.numberEdit === true) {
        throw new NativeGuiError(`NumberEdit Stage 1 Input${name} is Studio/Web only. Current Ready native ${PATCH_CURRENT_NATIVE_RUNTIME_VERSION} has no numeric-input presentation contract; validation fails closed rather than lowering it as a plain text Input.`);
      }
    }
