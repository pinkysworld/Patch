...
test('Studio Ready Window builds lower Native GUI IR 1.2 and seal current payload v12', () => {
  assert.match(studio, /buildNativeGuiIRV12 as buildNativeGuiIR/);
  assert.match(studio, /PATCH_SEALED_NATIVE_GUI_TREE_VERSION/);
  assert.match(studio, /sealNativeGuiRuntimeV12\(runtimeBytes, nativeGui, \{ platform: 'windows' \}\)/);
  assert.match(studio, /buildLinuxNativeGuiPackage\(runtimeBytes, nativeGui, \{ name, payloadVersion: PATCH_SEALED_NATIVE_GUI_TREE_VERSION \}\)/);
  assert.match(studio, /buildMacosNativeGuiPackage\(runtimeBytes, nativeGui, \{ name, payloadVersion: PATCH_SEALED_NATIVE_GUI_TREE_VERSION \}\)/);
  assert.match(studio, /allowMenuDecorations: true/);
  assert.match(studio, /allowTree: true/);
});

test('Studio site and offline PWA cache contain the complete current Native GUI IR 1.2 browser dependency chain', () => {
  for (const module of ['native-gui-ir-v08.js','native-gui-ir-v09.js','native-gui-ir-v10.js','native-gui-ir-v11.js','native-gui-ir-v12.js','native-tree-backend-adapter.js']) {
  ...
  assert.ok(siteBuilder.includes("'sealed-native-gui-v12.js'"), 'site builder missing current v12 sealer');
});

test('sealed native package helpers route payload v12 to the current sealer and preserve explicit v11 and older versions', () => {
...
test('Pages waits for and pins all three current TreeView-capable runtime v1.3 releases while retaining compatibility triggers', () => {
  ...
  assert.match(pages, /native-win32-runtime-v1\.3/);
  assert.match(pages, /native-linux-runtime-v1\.3/);
  assert.match(pages, /native-macos-runtime-v1\.3/);
