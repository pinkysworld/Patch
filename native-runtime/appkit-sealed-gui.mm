...
struct Form { std::string id,title; int width=640,height=420; bool visible=false; NSWindow* window=nil; std::vector<int> controls; };
...
static bool ReadSelfPayload(std::vector<uint8_t>&payload){
  std::string path;if(!SelfPath(path))return false;std::ifstream file(path,std::ios::binary|std::ios::ate);if(!file)return false;std::streamoff size=file.tellg();if(size<20)return false;file.seekg(size-20);uint8_t footer[20]{};file.read(reinterpret_cast<char*>(footer),20);if(!file||memcmp(footer,PATCH_MAGIC,8)!=0)return false;
  auto le32=[](const uint8_t*p){return(uint32_t)p[0]|((uint32_t)p[1]<<8)|((uint32_t)p[2]<<16)|((uint32_t)p[3]<<24);};uint32_t version=le32(footer+8),length=le32(footer+12),crc=le32(footer+16);if(version!=PATCH_PAYLOAD_VERSION||!length||(uint64_t)length>(uint64_t)(size-20))return false;file.seekg(size-20-(std::streamoff)length);payload.resize(length);file.read(reinterpret_cast<char*>(payload.data()),(std::streamsize)length);return file&&Crc32(payload.data(),payload.size())==crc;
}
...
static bool ParsePayload(const std::vector<uint8_t>&bytes){
  try{
    ...
    for(uint32_t f=0;f<formCount;++f){
      ...
      for(uint32_t c=0;c<count;++c){Control control;control.kind=r.u8();control.id=r.text();control.text=r.text();control.binding=r.text();uint32_t options=r.u32();if(options>10000)return false;for(uint32_t o=0;o<options;++o)control.options.push_back(r.text());control.x=r.i32();control.y=r.i32();control.width=r.i32();control.height=r.i32();control.parentTabIndex=r.i32();control.pageIndex=r.i32();control.formIndex=formIndex;control.commandId=nextCommand++;
        if(control.kind<CK_TEXT||control.kind>CK_TABS||control.width<=0||control.height<=0||control.width>10000||control.height>10000)return false;if((control.kind==CK_COMBO||control.kind==CK_LISTBOX||control.kind==CK_TABS)&&control.options.size()<2)return false;
        if(!control.id.empty()){if(gControlById.count(control.id))return false;gControlById[control.id]=(int)gControls.size();}form.controls.push_back((int)gControls.size());gControls.push_back(std::move(control));
      }
    ...
    gTabPages.resize(gControls.size(),nil);return true;
  }catch(...){return false;}
}
...
static void ExecuteEvent(const Event&event,bool eventBool,const std::string&eventText){for(const auto&action:event.actions){if(action.kind==ACT_OPEN)[gForms[gFormById[action.form]].window makeKeyAndOrderFront:nil];else if(action.kind==ACT_CLOSE)[gForms[gFormById[action.form]].window orderOut:nil];else if(action.kind==ACT_CHANGE){State&state=gStates[gStateByName[action.target]];for(const auto&op:action.ops)ApplyOperation(state,op,eventBool,eventText);}}RefreshUI();}
static void DispatchControl(int index,uint8_t kind){if(gRefreshing||index<0||index>=(int)gControls.size())return;auto&control=gControls[index];if(control.id.empty())return;for(const auto&event:gEvents){if(event.control!=control.id||event.kind!=kind)continue;bool eventBool=false;std::string eventText;if(event.valueType==1&&control.kind==CK_CHECKBOX)eventBool=[(NSButton*)control.widget state]==NSControlStateValueOn;else if(event.valueType==2&&control.kind==CK_INPUT)eventText=UTF8([(NSTextField*)control.widget stringValue]);else if(event.valueType==2&&control.kind==CK_COMBO)eventText=UTF8([(NSPopUpButton*)control.widget titleOfSelectedItem]);else if(event.valueType==2&&control.kind==CK_LISTBOX){NSInteger row=[(NSTableView*)control.widget selectedRow];if(row>=0&&(size_t)row<control.options.size())eventText=control.options[(size_t)row];}ExecuteEvent(event,eventBool,eventText);}}
...
- (void)handleControl:(id)sender { NSInteger index=[sender tag]-1000;if(index<0||index>=(NSInteger)gControls.size())return;auto&c=gControls[(size_t)index];DispatchControl((int)index,(c.kind==CK_CHECKBOX||c.kind==CK_COMBO||c.kind==CK_LISTBOX)?EV_CHANGED:EV_CLICKED); }
- (void)controlTextDidChange:(NSNotification*)notification { NSInteger index=[(NSTextField*)notification.object tag]-1000;DispatchControl((int)index,EV_CHANGED); }
...
static NSView* CreateControl(int index){
  ...
  NSView*widget=nil;
  if(c.kind==CK_TEXT){NSTextField*field=[[NSTextField alloc]initWithFrame:rect];field.editable=NO;field.selectable=NO;field.bezeled=NO;field.drawsBackground=NO;widget=field;}
  else if(c.kind==CK_INPUT){NSTextField*field=[[NSTextField alloc]initWithFrame:rect];field.tag=1000+index;field.delegate=gEventTarget;widget=field;}
  else if(c.kind==CK_COMBO){NSPopUpButton*combo=[[NSPopUpButton alloc]initWithFrame:rect pullsDown:NO];for(const auto&o:c.options)[combo addItemWithTitle:NS(o)];combo.tag=1000+index;combo.target=gEventTarget;combo.action=@selector(handleControl:);widget=combo;}
  else if(c.kind==CK_LISTBOX){NSScrollView*scroll=[[NSScrollView alloc]initWithFrame:rect];scroll.hasVerticalScroller=YES;scroll.autohidesScrollers=YES;scroll.borderType=NSBezelBorder;NSTableView*table=[[NSTableView alloc]initWithFrame:scroll.bounds];NSTableColumn*column=[[NSTableColumn alloc]initWithIdentifier:@"value"];column.width=c.width;[table addTableColumn:column];table.headerView=nil;table.allowsMultipleSelection=NO;table.allowsEmptySelection=YES;table.tag=1000+index;table.dataSource=gEventTarget;table.delegate=gEventTarget;scroll.documentView=table;[target addSubview:scroll];widget=table;}
  else if(c.kind==CK_BUTTON||c.kind==CK_CHECKBOX){NSButton*button=[[NSButton alloc]initWithFrame:rect];button.tag=1000+index;button.target=gEventTarget;button.action=@selector(handleControl:);if(c.kind==CK_CHECKBOX)button.buttonType=NSButtonTypeSwitch;else{button.buttonType=NSButtonTypeMomentaryPushIn;button.bezelStyle=NSBezelStyleRounded;}widget=button;}
  else return nil;
  c.widget=widget;if(c.kind!=CK_LISTBOX)[target addSubview:widget];return widget;
}
...
static int RunSmoke(){
  auto mainIt=gFormById.find("main");int mainIndex=mainIt==gFormById.end()?0:mainIt->second;auto settingsForm=gFormById.find("settings");if(![gForms[mainIndex].window isVisible])return 70;if(settingsForm!=gFormById.end()&&[gForms[settingsForm->second].window isVisible])return 71;
  if(gControlById.count("open_settings")&&settingsForm!=gFormById.end()){if(!Click("open_settings")||![gForms[settingsForm->second].window isVisible])return 72;}
  if(gControlById.count("name")&&gStateByName.count("name")){auto&c=gControls[gControlById["name"]];if(c.kind==CK_INPUT){[(NSTextField*)c.widget setStringValue:@"Ada"];NSNotification*n=[NSNotification notificationWithName:NSControlTextDidChangeNotification object:c.widget];[gEventTarget controlTextDidChange:n];if(gStates[gStateByName["name"]].text!="Ada")return 73;}}
  ...
  if(gControlById.count("notifications")){auto&c=gControls[gControlById["notifications"]];auto s=gStateByName.find("notifications");if(c.kind!=CK_CHECKBOX||s==gStateByName.end())return 76;bool before=gStates[s->second].boolean;[(NSButton*)c.widget performClick:nil];Pump();if(gStates[s->second].boolean==before)return 77;}
  if(gControlById.count("size")){auto&c=gControls[gControlById["size"]];auto s=gStateByName.find("size");if(c.kind!=CK_COMBO||c.options.empty()||s==gStateByName.end())return 78;NSInteger last=(NSInteger)c.options.size()-1;[(NSPopUpButton*)c.widget selectItemAtIndex:last];[gEventTarget handleControl:c.widget];Pump();if(gStates[s->second].text!=c.options[(size_t)last])return 79;}
  if(gControlById.count("fruit")){auto&c=gControls[gControlById["fruit"]];auto s=gStateByName.find("fruit");if(c.kind!=CK_LISTBOX||c.options.empty()||s==gStateByName.end())return 80;NSInteger last=(NSInteger)c.options.size()-1;[(NSTableView*)c.widget selectRowIndexes:[NSIndexSet indexSetWithIndex:(NSUInteger)last] byExtendingSelection:NO];[gEventTarget handleControl:c.widget];Pump();if(gStates[s->second].text!=c.options[(size_t)last])return 81;}
  if(gControlById.count("close_settings")&&settingsForm!=gFormById.end()){if(!Click("close_settings")||[gForms[settingsForm->second].window isVisible])return 82;}
  return 0;
}
...
int main(int argc,const char*argv[]){@autoreleasepool{[NSApplication sharedApplication];[NSApp setActivationPolicy:NSApplicationActivationPolicyRegular];std::vector<uint8_t>payload;if(!ReadSelfPayload(payload)||!ParsePayload(payload))return 20;gEventTarget=[PatchEventTarget new];gWindowDelegates=[NSMutableArray arrayWithCapacity:gForms.size()];if(!CreateForms())return 21;RefreshUI();[NSApp finishLaunching];for(auto&f:gForms)if(f.visible)[f.window makeKeyAndOrderFront:nil];if(HasArg(argc,argv,"--patch-smoke"))return RunSmoke();[NSApp activateIgnoringOtherApps:YES];[NSApp run];}return 0;}
