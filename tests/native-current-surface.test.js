...
test('browser packaging ships current and frozen contracts without retired v07-v11 copies', () => {
  ...
  for (const version of ['v12','v13','v14','v15','v16']) assert.match(buildSite, new RegExp('native-gui-ir-' + version + '\\.js'));
...
test('native compatibility documentation makes current versus frozen ownership explicit', () => {
  ...
  for (const marker of [
    'Native GUI IR 1.6 / sealed payload v16 / runtime v1.7',
    'native-current-contract.js',
    'native-gui-1.6/payload-16/runtime-1.7',
...
test('README and public website expose the native contract boundary without requiring identical prose', () => {
  ...
  assert.match(downloads, /Native GUI IR <strong>1\.6<\/strong>/);
  assert.match(downloads, /payload <strong>v16<\/strong>/);
  assert.match(downloads, /runtime <strong>v1\.7<\/strong>/);
