...
export function adaptNativeImageListForV18Backend(input) {
  ...
  for (let index = 0; index < originalControls.length; index += 1) {
    ...
    const width = Number(original.imageWidth);
    const height = Number(original.imageHeight);
    if (!Number.isInteger(width) || !Number.isInteger(height)) {
      throw new NativeGuiError(`Native Button '${original.id}' ImageList dimensions are invalid.`);
    }
    buttons.push({
      nativeIndex: index,
      id: original.id,
      ...
      resourceId: String(original.imageResourceId),
