...
test('browser package seals current Native GUI IR 1.6 payload v16 into a minimal macOS app bundle ZIP', () => {
  assert.equal(gui.version, '1.6');
...
test('Studio defaults macOS Window downloads to native AppKit runtime v1.7 sealing without token', () => {
  ...
  assert.ok(studio.includes('macOS native AppKit runtime v1.7 app downloaded · unsigned · no token · no Electron'));
  assert.ok(studio.includes('Native GUI IR 1.6'));
  assert.ok(studio.includes('payload v16'));
