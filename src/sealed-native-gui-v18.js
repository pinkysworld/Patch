...
export function inspectNativeGuiImageListsV18(payloadBytes) {
  ...
  for (let index = 0; index < listCount; index += 1) {
    ...
    if (!id || ids.has(id) || itemCount > MAX_ITEMS) {
  ...
  for (let index = 0; index < buttonCount; index += 1) {
    ...
    if (!ids.has(imageListId)) throw new SealedNativeGuiV18Error(`Payload v18 Button '${id}' refers to a missing ImageList.`);
    buttonIds.add(id);
...
class Reader {
  ...
  need(count) { if (count < 0 || this.offset + count > this.bytes.length) throw new SealedNativeGuiV18Error('Malformed payload v18 ImageList bytes.'); }
  u32() { this.need(4); const value = this.view.getUint32(this.offset, true); this.offset += 4; return value; }
  text() { const length = this.u32(); this.need(length); let value; try { value = new TextDecoder('utf-8', { fatal: true }).decode(this.bytes.subarray(this.offset, this.offset + length)); } catch { throw new SealedNativeGuiV18Error('Payload v18 contains invalid UTF-8 text.'); } this.offset += length; return value; }
