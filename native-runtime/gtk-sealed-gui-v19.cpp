...
static GdkPixbuf* PatchButtonPixbufV19(const std::string& source, int width, int height) {
  ...
  if (decoded && gdk_pixbuf_get_width(decoded) > 0 && gdk_pixbuf_get_height(decoded) > 0) {
    scaled = gdk_pixbuf_scale_simple(decoded, width, height, GDK_INTERP_BILINEAR);
  }
...
static int RunPatchImageListSmokeV19() {
  ...
  for (const auto& item : gPatchButtonImagesV19) {
    ...
    GtkWidget* image = gtk_button_get_image(GTK_BUTTON(c.widget));
