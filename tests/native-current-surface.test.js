...
test('browser packaging ships current and frozen contracts without retired v07-v11 copies', () => {
  ...
  for (const version of ['v12','v13','v14','v15','v16','v17','v18','v19']) assert.match(buildSite, new RegExp('native-gui-ir-' + version + '\\.js'));
...
test('native compatibility documentation makes current versus frozen ownership explicit', () => {
  ...
  for (const marker of [
    'Native GUI IR 1.9 / sealed payload v19 / runtime v1.10',
    'native-current-contract.js',
    'native-gui-1.9/payload-19/runtime-1.10',
    'payload v17 / runtime v1.8',
...
test('README and public website expose the native contract boundary without requiring identical prose', () => {
  ...
  assert.match(downloads, /Native GUI IR 1\.7 \/ payload v17 \/ runtime v1\.8 remains an explicit Offline Compiler compatibility line/);
  assert.match(downloads, /Native GUI IR 1\.3 \/ payload v13 \/ runtime v1\.4 is the Slider-capable historical line/);
});
