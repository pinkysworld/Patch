...
test('capability matrix is generated from the canonical registry rather than a second catalog', () => {
  ...
  assert.equal(matrix.contract.id, 'native-gui-1.8/payload-18/runtime-1.9');
  assert.equal(matrix.contract.nativeGuiIR, '1.8');
  assert.equal(matrix.contract.payload, 18);
  assert.equal(matrix.contract.runtime, '1.9');
  ...
  assert.equal(imagelist.targets.windows, 'supported');
...
test('patch components CLI prints the registry matrix and JSON envelope', () => {
  ...
  assert.equal(report.contract.id, 'native-gui-1.8/payload-18/runtime-1.9');
