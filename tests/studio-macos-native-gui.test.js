...
test('browser package seals current Native GUI IR 1.5 payload v15 into a minimal macOS app bundle ZIP', () => {
  assert.equal(gui.version, '1.5');
...
test('Studio defaults macOS Window downloads to native AppKit runtime v1.6 sealing without token', () => {
  ...
  assert.ok(studio.includes('macOS native AppKit runtime v1.6 app downloaded · unsigned · no token · no Electron'));
  assert.ok(studio.includes('Native GUI IR 1.5'));
  assert.ok(studio.includes('payload v15'));
