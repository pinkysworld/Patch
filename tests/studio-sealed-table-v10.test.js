...
test('Studio Ready Window builds use the stable current facade for Native GUI IR 1.5 and payload v15', () => {
  ...
  assert.match(current, /PATCH_CURRENT_NATIVE_CONTRACT_ID = 'native-gui-1\.5\/payload-15\/runtime-1\.6'/);
  assert.match(current, /buildNativeGuiIRV15/);
  assert.match(current, /sealNativeGuiRuntimeV15/);
...
test('Studio site and offline PWA cache contain current v15 plus frozen compatibility modules without retired v07-v11 copies', () => {
  ...
  assert.ok(siteBuilder.includes("'sealed-native-gui-v15.js'"), 'site builder missing current v15 implementation sealer');
});

test('sealed native package helpers route payload v15 and frozen v12 through live facades', () => {
...
test('Pages waits for current Shape v1.6 and frozen runtime lines without retired v07-v11 sealed workflows', () => {
  ...
  assert.match(pages, /native-win32-runtime-v1\.6/);
  assert.match(pages, /native-linux-runtime-v1\.6/);
  assert.match(pages, /native-macos-runtime-v1\.6/);
  assert.doesNotMatch(pages, /WIN32_RUNTIME_TAG: native-win32-runtime-v1\.5/);
  assert.doesNotMatch(pages, /LINUX_NATIVE_RUNTIME_TAG: native-linux-runtime-v1\.5/);
  assert.doesNotMatch(pages, /MACOS_NATIVE_RUNTIME_TAG: native-macos-runtime-v1\.5/);
