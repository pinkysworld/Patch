...
test('capability matrix is generated from the canonical registry rather than a second catalog', () => {
  ...
  assert.equal(matrix.contract.id, 'native-gui-1.7/payload-17/runtime-1.8');
  assert.equal(matrix.contract.nativeGuiIR, '1.7');
  assert.equal(matrix.contract.payload, 17);
  assert.equal(matrix.contract.runtime, '1.8');
