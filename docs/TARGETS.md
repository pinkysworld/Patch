...
Current product baseline: **0.2.0-beta.36** · Change IR **0.10** · Native GUI IR **1.8** · sealed desktop payload **v18** / runtime **v1.9**.
...
| Window, token-free Ready/offline | payload v18 / runtime v1.9 | payload v18 / runtime v1.9 | payload v18 / runtime v1.9 | unsupported | Standalone Web |
...
The current Window surface includes source-backed Form geometry and responsive Anchor/Dock policies, Text/Button/Input/Checkbox/ComboBox/ListBox/Radio/Tabs, Table/Grid, menus/dialogs, persistent list-backed multi-select ListBox, hierarchical TreeView, Slider, Chrome Stage 1 Panel/Timer/PictureBox/StatusBar, Shape Stage 1, PaintBox Stage 1, PaintBox `draw image` and native ImageList Button images. Unsupported native constructs fail closed rather than selecting Electron implicitly.
...
### Current Native GUI IR 1.8 / payload v18 / runtime v1.9
...
Native GUI IR **1.8** adds ImageList Button `image list.item` PNG/JPEG images while preserving PaintBox `draw image`, PaintBox Stage 1, Shape, Slider, Chrome Stage 1, TreeView, Table, ListBox and Menu semantics. Current token-free Ready apps and ordinary offline `patch link` on Windows, macOS and Linux seal **payload v18** into **runtime v1.9**.
...
TreeView mappings remain common-controls TreeView, `NSOutlineView`, and `GtkTreeView` + `GtkTreeStore` respectively. Shape mappings are Win32 GDI+, AppKit `NSBezierPath` and GTK cairo in 0–100 space with device-pixel strokes. PaintBox mappings use the same host drawing APIs in source-control logical pixels, scaled to the client size.

The v1.9 runtime workflow builds, seals and executes the canonical ImageList program independently on all three desktop hosts and verifies that the existing PaintBox/Shape/Chrome/Slider/Table/ListBox/Menu/Tree action engine remains intact.
...
- Window Ready/offline: native Win32 runtime v1.9 consuming payload v18 / Native GUI IR 1.8;
...
- Window Ready/offline: AppKit runtime v1.9 consuming payload v18 / Native GUI IR 1.8;
...
- Intel offline compiler: portable kit with bundled Intel Node runtime and x86-64 AppKit runtime v1.9;
...
- Window Ready/offline: GTK3 runtime v1.9 consuming payload v18 / Native GUI IR 1.8;
