...
test('transitive witness preserves each nested edge as its own exact witness too', () => {
  const compiled = compile(source, { name: 'TransitiveCalls' });
  const artifact = buildTransitiveCallBodyWitnesses(compiled.ast, compiled.ir.formalCalls);
...
test('transitive producer fails closed when a nested argument interval does not fit the declaration', () => {
...
do caller(1)`;
  assert.throws(() => compile(invalid, { name: 'InvalidTransitiveFit' }), /fit|range|argument|interval/i);
});
