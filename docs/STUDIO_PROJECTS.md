...
Project bundle v4 establishes deterministic portable project resources. It does not imply that every resource-consuming control has cross-target parity. Target capability gates remain component-specific: current native Picture follows `native-picture-formats/1.0` (Ready PNG/JPEG, deferred WebP/SVG), Shape and PaintBox Stage 1 native parity is current; PaintBox `draw image` remains open, ImageList is Web metadata for Button `image list.item` while native GUI IR 1.4 still fail-closes ImageList and Button images, and Window icons package on Studio/Web under `window-icon/1.0` while native GUI IR 1.4 fail-closes Form icons.
