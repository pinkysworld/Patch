...
test('Pages consumes current ImageList runtime v1.9 and frozen historical accessibility lines separately', () => {
  assert.match(pagesWorkflow, /WIN32_RUNTIME_TAG: native-win32-runtime-v1\.9/);
  assert.match(pagesWorkflow, /LINUX_NATIVE_RUNTIME_TAG: native-linux-runtime-v1\.9/);
  assert.match(pagesWorkflow, /MACOS_NATIVE_RUNTIME_TAG: native-macos-runtime-v1\.9/);
  ...
  assert.match(pagesWorkflow, /Patch Native Sealed Chrome Runtime v1\.5/);
  assert.match(pagesWorkflow, /Patch Native Sealed Shape Runtime v1\.6/);
  assert.match(pagesWorkflow, /Patch Native Sealed PaintBox Runtime v1\.7/);
  assert.match(pagesWorkflow, /Patch Native Sealed PaintBox Image Runtime v1\.8/);
  assert.match(pagesWorkflow, /Patch Native Sealed ImageList Runtime v1\.9/);
