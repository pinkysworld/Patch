...
Project bundle v4 establishes deterministic portable project resources. It does not imply that every resource-consuming control has cross-target parity. Target capability gates remain component-specific: current native Picture follows `native-picture-formats/1.0` (Ready PNG/JPEG, deferred WebP/SVG), Shape, PaintBox Stage 1 and PaintBox `draw image` native parity is current; ImageList is Web and native Button `image list.item` PNG/JPEG metadata on Native GUI IR 1.8 while ToolBar/TreeView image bindings remain later, and Window icons package on Studio/Web under `window-icon/1.0` while native GUI IR 1.4 fail-closes Form icons.
