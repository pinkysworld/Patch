...
import { decodeNativeGuiPayloadV18, inspectNativeGuiPaintImagesV18, inspectNativeGuiChromeV18, inspectNativeGuiSlidersV18 } from '../src/sealed-native-gui-v18.js';
...
function currentPayloadV16(bytes) {
  return inspectNativeGuiPaintImagesV18(decodeNativeGuiPayloadV18(bytes)).payloadV16;
}
...
test('offline linker lowers current Window source to Native GUI IR 1.8 and seals payload v18', () => {
  ...
  for (const [platform, runtime, suggestedOutput] of cases) {
    ...
    assert.ok(decodeNativeGuiPayloadV18(executable.bytes).length > 0);
    assert.equal(footerVersion(executable.bytes), 18);
  }
});

test('offline Window linker preserves Table in the current payload v18 contract', () => {
...
test('offline Window linker preserves persistent list state and multi-select ListBox in payload v18', () => {
  for (const [platform, runtime] of [
    ['windows', Uint8Array.from([0x4d, 0x5a, 0, 0])],
    ['linux', Uint8Array.from([0x7f, 0x45, 0x4c, 0x46, 0])],
    ['macos', Uint8Array.from([0xcf, 0xfa, 0xed, 0xfe, 0])]
...
test('offline Window linker preserves decorated Menu metadata in payload v18', () => {
  for (const [platform, runtime] of [
    ['windows', Uint8Array.from([0x4d,0x5a,0,0])],
    ['linux', Uint8Array.from([0x7f,0x45,0x4c,0x46,0])],
    ['macos', Uint8Array.from([0xcf,0xfa,0xed,0xfe,0])]
...
test('offline Window linker supports hierarchical TreeView through payload v18 on all Ready desktop hosts', () => {
  for (const [platform, runtime] of [
    ['windows', Uint8Array.from([0x4d,0x5a,0,0])],
    ['linux', Uint8Array.from([0x7f,0x45,0x4c,0x46,0])],
    ['macos', Uint8Array.from([0xcf,0xfa,0xed,0xfe,0])]
...
test('offline Window linker supports native Slider numeric events in payload v18', () => {
  for (const [platform, runtime] of [
    ['windows', Uint8Array.from([0x4d,0x5a,0,0])],
    ['linux', Uint8Array.from([0x7f,0x45,0x4c,0x46,0])],
    ['macos', Uint8Array.from([0xcf,0xfa,0xed,0xfe,0])]
    ...
    const executable = executableFrom(plan, platform);
    const metadata = inspectNativeGuiSlidersV18(decodeNativeGuiPayloadV18(executable.bytes));
    assert.equal(footerVersion(executable.bytes), 18);
...
test('offline Window linker carries Panel children, Timer ticked and PictureBox clicked through payload v18', () => {
  for (const [platform, runtime] of [
    ['windows', Uint8Array.from([0x4d,0x5a,0,0])],
    ['linux', Uint8Array.from([0x7f,0x45,0x4c,0x46,0])],
    ['macos', Uint8Array.from([0xcf,0xfa,0xed,0xfe,0])]
    ...
    const executable = executableFrom(plan, platform);
    const inspected = inspectNativeGuiChromeV18(decodeNativeGuiPayloadV18(executable.bytes));
    assert.equal(footerVersion(executable.bytes), 18);
...
test('offline linker preserves explicit payload v17 and v12 compatibility while current defaults to v18', () => {
  const runtime = Uint8Array.from([0x4d, 0x5a, 0, 0]);
  const v17 = createOfflineLinkPlan(windowSource, {
    platform: 'windows', name: 'LegacyWindow17', guiRuntime: runtime, guiPayloadVersion: 17
  });
